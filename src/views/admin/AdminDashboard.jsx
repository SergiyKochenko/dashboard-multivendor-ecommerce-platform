...
const AdminDashboard = () => {
  ...
  const { totalSale, totalOrder, totalProduct, totalSeller, recentOrder, recentMessage } =
  ...
  const state = {
    series: [
      {
        name: 'Orders',
        data: [25, 38, 47, 53, 80, 35, 27, 79, 90, 75, 40, 35],
      },
      {
        name: 'Revenue',
        data: [60, 40, 55, 60, 91, 55, 25, 70, 80, 77, 66, 77],
      },
      {
        name: 'Sellers',
        data: [35, 36, 55, 58, 85, 66, 33, 66, 99, 70, 40, 55],
      },
    ],
    options: {
      color: ['#181ee8', '#181ee8'],
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-7">
        <div className="flex justify-between items-center p-5 bg-[#fae8e8] rounded-md gap-3">
          ...
          <div className="w-[40px] h-[47px] rounded-full bg-[#fa0305] flex justify-center items-center text-3xl">
            <FaEuroSign className="text-[#fae8e8] shadow-lg" />
          </div>
        </div>
                    ...
                    <li className="mb-3 ml-6" key={i}>
                      <div className="flex absolute -left-5 shadow-lg justify-center items-center w-10 h-10 p-[6px] bg-[#4c7fe2] rounded-full z-10">
                        <img
                          ...
                          src={m.image || seller}
                          alt={m.senderName || 'seller'}
                        />
                      </div>
                      <div className="p-3 bg-slate-800 rounded-lg border border-slate-600 shadow-sm">
                        <div className="flex justify-between items-center mb-2">
                          <Link
                            to={m.senderId ? `/admin/dashboard/chat-sellers/${m.senderId}` : '/admin/dashboard/chat-sellers'}
                            className="text-md font-normal"
                          >
                            {m.senderName || 'Seller'}
                          </Link>
                          <time className="mb-1 text-sm font-normal sm:order-last sm:mb-0">
                            {m.createdAt ? moment(m.createdAt).fromNow() : ''}
                          </time>
                        </div>
